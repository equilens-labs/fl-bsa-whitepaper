...
\label{eq:air}
\end{equation}
For gender (binary), we report a single pair (\GenderProtectedGroup{} vs \GenderReferenceGroup{}). For race (multi-class), the configured reference group is provenance, while the effective pairwise screening comparison may be selected separately under the recorded highest-rate policy. The generated intake is authoritative for both fields. The main body reports the worst-case pair only when the small-n display policy is satisfied.

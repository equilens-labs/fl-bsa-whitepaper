...
The 21 intake certificates each contain a canonical content hash; 20 predecessor fields resolve to
another bundled certificate hash and one certificate is the chain root. The bundle also carries
signature-shaped metadata. Its public verification key is not in the whitepaper companion, and the
intake intent does not expect this intake pack to be cryptographically authoritative. Accordingly,
this paper claims \emph{integrity linkage}, not independent tamper proof. The companion ZIP digest
on the cover becomes an external check only when a reviewer receives that digest through a trusted
channel.

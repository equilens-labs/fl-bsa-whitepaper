...
If you’d like, I can now:
	•	Propose exact LaTeX macro definitions / template changes to fix the Snapshot block (including the not-evaluated path), or
	•	Draft a Gate-WP post-processing script that reads metrics_long.csv + manifest.json and emits the summary JSON/macros the LaTeX currently expects.
...
def build_summary(root: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "flbsa.whitepaper_characterization.v1",
...
def _write_macros(summary: dict[str, Any], output: Path) -> None:
    ...
    macros = {
        ...
        "UtilityRetentionMean": _fmt(bands["roc_auc_retention"]["mean"]),
        "UtilityRetentionMin": _fmt(bands["roc_auc_retention"]["min"]),
        "UtilityRetentionMax": _fmt(bands["roc_auc_retention"]["max"]),
    }
...
def _write_tables(summary: dict[str, Any], outdir: Path) -> None:
    slice_lines = [
        ...
        r"\toprule",
        r"Slice & $n_{prot}$ & $n_{ref}$ & AIR & 95\% CI & Relation to 0.80 \\",
        r"\midrule",
    ]
    for row in summary["fairness"]["slices"]:
        relation = f"point {row['point_screen_relation']}; CI {row['interval_screen_relation']}"
        slice_lines.append(
...
def _write_plots(summary: dict[str, Any], outdir: Path) -> None:
    ...
    ax.set_title("Single-run branch characterization (95% conditional intervals)")

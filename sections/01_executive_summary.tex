...
\begin{KeyBox}
\begin{KeyValueList}
\item[Fairness signal] Historical AIR \HistoricalAIR{} and amplification AIR \AmplificationAIR{}
are both below the internally selected 0.80 point screen. The amplification 95\% interval is
[\AmplificationAIRLower{}, \AmplificationAIRUpper{}].
\item[Branch control] Intrinsic AIR \IntrinsicAIR{} [\IntrinsicAIRLower{}, \IntrinsicAIRUpper{}]
...
\item[Quality] Overall quality is \AmplificationQuality{} for amplification and
\IntrinsicQuality{} for intrinsic. The producer's certificate utility field is unevaluated for
both branches.
\item[Robustness] \RobustnessTotalPasses{}/\RobustnessTotalRuns{} configured Gold scenario runs
passed across ten seeds and four scenarios. This is gate stability, not an interval for a
population parameter.
\item[Utility study] Ten equal-size train-on-synthetic runs yield mean ROC AUC
...
characterization or retire it. The annotated v5.0.2 candidate tag exists at tag object
...
31087235319; its lock pins \texttt{cryptography} 50.0.0. The full Release workflow run 31089912581
later failed on that same commit, and no v5.0.2 GitHub Release had published as of 6 August 2026.
Thus v5.0.2 remediates the affected dependency but is not a published-current-release anchor. This
paper does not characterize v5.0.2 evidence and must never be relabelled as v5.0.2. A replacement
current-release paper must wait for, and then bind to, whichever exact product tag completes the
full Release workflow and actually publishes; the successor version cannot be forecast while this
paper is drafted.

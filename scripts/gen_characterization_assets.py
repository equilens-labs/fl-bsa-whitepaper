...
def _read_digest_anchored_bytes(path: Path, expected_sha256: str) -> bytes:
    try:
...
def _read_digest_anchored_json(
    ...
    data = _read_digest_anchored_bytes(path, expected_sha256)
    try:
...
def build_summary(root: Path) -> dict[str, Any]:
    ...
    _read_digest_anchored_bytes(
        root / GOLD_EVIDENCE_MANIFEST_PATH,
        GOLD_EVIDENCE_MANIFEST_SHA256,
    )
    _read_digest_anchored_bytes(root / GOLD_INDEX_PATH, GOLD_INDEX_SHA256)
    robustness = _read_digest_anchored_json(
        root / GOLD_SUMMARY_PATH, GOLD_SUMMARY_SHA256
    )
...
def _write_macros(summary: dict[str, Any], output: Path) -> None:
    ...
    macros = {
        ...
        "GoldEvidenceManifestShaRaw": GOLD_EVIDENCE_MANIFEST_SHA256,
        "GoldIndexShaRaw": GOLD_INDEX_SHA256,
        "GoldSummaryShaRaw": GOLD_SUMMARY_SHA256,
    }
